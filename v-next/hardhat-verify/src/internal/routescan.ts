...
export const ROUTESCAN_PROVIDER_NAME: keyof VerificationProvidersConfig =
  "routescan";

const VERIFICATION_STATUS_POLLING_SECONDS = 3;

export class Routescan implements VerificationProvider {
  public name: string;
  public url: string;
  ...
  constructor(routescanConfig: {
    name?: string;
    url: string;
    apiUrl: string;
    dispatcher?: Dispatcher;
  }) {
    this.name = routescanConfig.name ?? "Routescan";
    this.url = routescanConfig.url;
    this.apiUrl = routescanConfig.apiUrl;
    this.#dispatcher = routescanConfig.dispatcher;
    this.#pollingIntervalMs =
      routescanConfig.dispatcher !== undefined
        ? 0
  ...
  public async verify(
    ...
    const routescanResponse = new RoutescanVerificationResponse(responseBody);

    if (routescanResponse.isBytecodeMissingInNetworkError()) {
      throw new HardhatError(
    ...
    if (routescanResponse.isAlreadyVerified()) {
      throw new HardhatError(
    ...
    if (!routescanResponse.isOk()) {
      throw new HardhatError(
        HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL.CONTRACT_VERIFICATION_REQUEST_FAILED,
        { message: routescanResponse.message },
      );
    }

    return routescanResponse.message;
  }
  ...
  public async pollVerificationStatus(
    ...
    const routescanResponse = new RoutescanVerificationStatusResponse(
      responseBody,
    );

    if (routescanResponse.isPending()) {
    ...
    if (!(routescanResponse.isFailure() || routescanResponse.isSuccess())) {
    ...
    return {
      success: routescanResponse.isSuccess(),
      message: routescanResponse.message,
    };
  }
}

class RoutescanVerificationResponse implements VerificationResponse {
...
class RoutescanVerificationStatusResponse
